...
| Attendees | FCPS IT Lead, C&T Project Lead, C&T Tech Lead |
...
Technical deep-dive with the FCPS IT Lead. Goal: understand the existing infrastructure,
confirm the Oracle schema, validate the ID.me integration approach, and establish what
the C&T team will build vs what FCPS IT will provide.
...
- SSH access: FCPS IT Lead will provide the key pair to C&T before development starts.
...
- FCPS IT Lead confirmed the schema C&T needs to create: STAFF and PROCUREMENT_ITEMS tables.
...
- ID.me callback must be a publicly reachable URL. FCPS IT Lead will whitelist the EC2 IP.

---

## ID.me Integration

### What FCPS IT Lead confirmed

- FCPS has an existing ID.me organisational account.
...
- Scopes needed: `openid email`. FCPS IT Lead confirmed these are sufficient to
...
- Redirect URI must be registered in the ID.me developer console exactly as it will
  ...
  C&T to confirm the exact URI; FCPS IT Lead registers it.
...
FCPS IT Lead flagged: if the redirect URI in the app does not exactly match what is
...
FCPS IT Lead reviewed and approved the schema in `docs/DATA_MODEL.md`. Specific confirmations:
...
- Seed data: C&T creates its own synthetic seed data for the demo. Real FCPS staff records
...
- SCREAMING_SNAKE_CASE naming for all Oracle objects — confirmed matches FCPS convention.
...
| EC2_SSH_KEY | High | FCPS IT Lead holds original. C&T receives a copy via 1Password. |
| STAFF seed data | PII (synthetic) | Fictional names and IDs only. Never real FCPS employee data. |
...
- **C&T builds on local Docker Compose, deploys to the existing EC2 via GitHub Actions.**
  FCPS IT Lead does not want C&T managing the EC2 directly — deploy via SSH in CI only.
- **ID.me sandbox for the demo.** Production ID.me integration is phase 2.
- **No real FCPS staff data in the demo environment at any point.**
  Seed data will be synthetic. Confirmed by FCPS IT Lead.
- **Oracle schema managed by `seed_oracle.py`.** No migration tool — too heavyweight for a demo.
- **oracledb thin mode.** No Oracle Client needed. Confirmed as viable by FCPS IT Lead.
...
- [ ] Exact EC2 public IP — needed to register the ID.me redirect URI. Owner: FCPS IT Lead — by 2026-05-08.
- [ ] 1Password vault access for C&T team — needed before first ID.me integration work. Owner: FCPS IT Lead — by 2026-05-08.
- [ ] GitHub Actions SSH deploy — does FCPS IT firewall allow inbound SSH from GitHub Actions IPs? Owner: FCPS IT Lead — by 2026-05-10.
- [ ] Oracle XE service name on the EC2 — is it `XEPDB1` or a custom name? Owner: FCPS IT Lead — by 2026-05-08.
...
> can see vendor bank account numbers, we have a serious problem." — FCPS IT Lead

> "Do not touch the production Oracle instance. Not even to look at it.
> Use your own container for development." — FCPS IT Lead
...
|---|---|---|
| Share EC2 public IP | FCPS IT Lead | 2026-05-08 |
| Share ID.me sandbox credentials via 1Password | FCPS IT Lead | 2026-05-08 |
| Confirm Oracle XE service name on EC2 | FCPS IT Lead | 2026-05-08 |
| Confirm GitHub Actions SSH access through FCPS firewall | FCPS IT Lead | 2026-05-10 |
| Register ID.me redirect URI once EC2 IP confirmed | FCPS IT Lead + C&T Tech Lead | 2026-05-12 |
...
- FCPS IT Lead joined 5 minutes late. Started with a walkthrough of the EC2 console.
...
- Oracle XE container was started manually. No systemd service yet — FCPS IT Lead
  will add auto-start before the demo.
- ID.me developer console access: FCPS IT Lead owns the account. C&T does not get
  direct console access — all changes go through FCPS IT Lead.
- FCPS IT Lead was very clear: "I trust C&T to build it, but I need to control what
